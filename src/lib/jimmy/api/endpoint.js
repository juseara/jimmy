...
import { response, responseFor, responseForError, respondNotFound, respondUnauthorized } from 'jimmy/helpers/response'
...
export default class Endpoint {
    ...
    get params(){
        if(!this._params){
            this._params = {...this.req.query, ...this.req.body, ...this.req.params };
        }
    ...
    dispatch(name, ...args){
        ...
        function respond(result) {
			...
			function responder() {
				if (result.object) {
					if(typeof Jimmy.responders[result.responder] == 'function') {
						return Jimmy.responders[result.responder](result.object, this.context);
					}
				}
			}
